...
from utils import *
import params
print("all imported except yolo")
#Add yolo
import ultralytics
...
class pose_annotation_app:
    ...
    def in_img_left_range(self, row, col): #주어진 행(row)과 열(col)이 이미지 영역 내에 있는지 확인
    ...
    def yolo_crop_box(self):
        # 자동으로 Set crop box (Yolo predict)
        model = YOLO('.\\yolo_weights\\weights\\best.pt')
        # print("\n\n\n"+self.img_path_list[self.img_i]+"\n\n\n")

        # 사진 파일이 열려있으면 FileNotFoundError
        results = model('.\\'+ self.img_path_list[self.img_i])
        boxes = results[0].boxes
        box = boxes[0]  # returns one box

        # bounding box coordination
        x1, y1, x2, y2 = box.xyxy[0].tolist()
        crop_top, crop_left, crop_bot, crop_right = round(y1), round(x1), round(y2), round(x2)
        ...
        img_cropped = self.img_left[img_crop_top:img_crop_bot,
                                    img_crop_left:img_crop_right]
        ...
        crop_w = (img_crop_right - img_crop_left) + \
        ...
        self.img_right = cv2.resize(
            img_padded, (params.IMG_SIZE, params.IMG_SIZE))
    ...
    def mouse_right_click_callback(self, event): #수동으로 bounding box 생성
        row, col = event.y, event.x #마우스 클릭 위치의 행(row)과 열(col) 좌표를 가져옴
        if self.in_img_left_range(row, col):  # 클릭 좌표가 이미지 영역 내에 있는지 확인
            row -= params.IMG_PADDING
            col -= params.IMG_PADDING
